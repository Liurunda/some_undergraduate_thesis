...
#define BS_BAK_ROW_VBV        3
    ...
    while( 1 )
    {
        ...
        x264_macroblock_analyse_modified( h, roi_mvs_modified);
...
// one motion vector: (dx dy dt)  dt: choice of reference frame (dx,dy): choice of reference block in a frame
// when pass in, *mvs contain the old ROI region, and we want to check the motion vector inside ROI region
int     x264_encoder_encode_modified( x264_t *h,
                             x264_nal_t **pp_nal, int *pi_nal,
                             x264_picture_t *pic_in,
                             x264_picture_t *pic_out, int *mvs)//get the motion vectors in specific region
{
    x264_t *thread_current, *thread_prev, *thread_oldest;
    int i_nal_type, i_nal_ref_idc, i_global_qp;
