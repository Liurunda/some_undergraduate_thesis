...
static int *roi_mvs_modified;//[0lx,1rx,2ly,3ry]: rectangle [4]: cnt
...
static void mb_analyse_inter_p16x16( x264_t *h, x264_mb_analysis_t *a )
{
    ...
    for( int i_ref = 0; i_ref < h->mb.pic.i_fref[0]; i_ref++ )
    {
        ...
        x264_mb_predict_mv_16x16( h, 0, i_ref, m.mvp );//需要解决：mv的比例尺 i_ref相隔几帧
...
void x264_macroblock_analyse_modified( x264_t *h , int* mvs)
{
    x264_mb_analysis_t analysis;
    int i_cost = COST_MAX;
    roi_mvs_modified = mvs;
    h->mb.i_qp = x264_ratecontrol_mb_qp( h );
    /* If the QP of this MB is within 1 of the previous MB, code the same QP as the previous MB,
     * to lower the bit cost of the qp_delta.  Don't do this if QPRD is enabled. */
    if( h->param.rc.i_aq_mode && h->param.analyse.i_subpel_refine < 10 )
...
skip_analysis:
        ...
        else
        {
            ...
            if(roi_mvs_modified[0]<= h->mb.i_mb_x && h->mb.i_mb_x <=roi_mvs_modified[1]\
                && roi_mvs_modified[2] <= h->mb.i_mb_y && h->mb.i_mb_y <= roi_mvs_modified[3])
             {
                for( int i = 0; i < h->mb.pic.i_fref[0]; i++ ){

                    int offset = 5 + roi_mvs_modified[4] * 2;
                    roi_mvs_modified[offset + 0] = i;//问题: 是否选择了相邻的帧做预测?
                    roi_mvs_modified[offset + 1] = M32(analysis.l0.mvc[i][0]);
                    roi_mvs_modified[4] += 1;
                }
             }
